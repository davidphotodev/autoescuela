			...
			<div class="w-95 margintb-20 white b-radius-7-5 padding-25 padding-15-sm">
				<div class="d-flex d-block-md">
					<div class="w-60 w-100-md paddingl-20">
						...
						<p><strong>Tipo de Test:</strong> Exámen Oficial</p>
					...
					<div class="w-40 w-100-md all-center d-block-md margint-35-md">
						<div class="w-50 w-85-sm">
							...
							<p>
								<div class="w-100 btn b-radius-25 red all-center">
									<span>{{ limitTimeLocal / 60 }}:00 | {{ test.limitTime | countdown }}</span><i class="material-icons">timer</i>
								</div>
							</p>
						</div>
					</div>
					...
					<div class="d-flex">
						...
						<div class="w-70 w-100-sm">
							<template>
								...
								<div>
									<div v-for="(item, index) in question.answers" @click="answerSelection(index)" ref="answerSelection" class="selectAnswer paddingtb-15 paddinglr-25 b-radius-25 margintb-15 grey lighten-4">
				...
				<div v-if="modal" class="modal-window all-center">
					<div class="content-modal white w-80 b-radius-10 paddinglr-40 paddingb-40">
						...
						<div class="center-align margin-auto margint-45">
							<div>
								<button @click="processTest" class="paddinglr-25 marginlr-10 btn b-radius-25 red">Ver Resultado</button>
							</div>
						</div>
...
export default {
  ...
  created(){
  	...
  	this.limitTimeLocal = this.test.limitTime;
  	let time = setInterval(() => {
  		if (this.test.limitTime > 0) {
  			this.test.limitTime--
  		}
  	}, 50)
	...
	this.questions.forEach( function(element, index) {
		element.answers.forEach( function(elem, index) {
			elem.selected = false
		});
	});
  ...
  methods: {
  	...
  	skipQuestion(){
  		let validate = true;
  		if(this.finishTest){
  			M.toast({html: 'Debe corregir el Test', classes: 'blue white-text'});
  	...
  	nextQuestion(){
  		let validate = true;
  		if (this.finishTest) {
  			M.toast({html: 'Debe finalizar el Test', classes: 'blue-grey lighten-5 black-text'});
  			this.$refs.btnFinish.classList.add('pulse-local');
  			this.$refs.btnFinish.classList.add('z-depth-2');
  			this.$refs.chipList.forEach((item) => item.classList.remove('b-chip'));
  			validate = false
  	...
  	processTest(){
  		...
  		this.$api.post('tests', this.test).then( res => {
  			...
  			this.questions.forEach( (item, index) =>  {
  				console.log(item.id)
  				item.answers.forEach( function(element, index) {
  ...
  filters: {
  	countdown(seconds){
  		...
  		let seg = parseInt(seconds % 60, 10);
  		return min + ":" + seg
  	}
  },
  watch: {
  	'test.limitTime'(value){
  		if (value == 1) {
  			this.modal = true;
  		}
  	}
