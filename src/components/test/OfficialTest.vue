			...
			<div class="w-95 margintb-20 white b-radius-7-5 padding-25 padding-15-sm">
				<div class="d-flex d-block-md">
					...
					<div class="w-40 w-100-md all-center d-block-md margint-35-md">
						<div class="w-50 w-85-sm">
							...
							<p>
								<div ref="timer" class="w-100 btn b-radius-25 grey darken-4 all-center">
				...
				<div v-if="modal" class="modal-window all-center">
					<div class="content-modal white w-50 w-70-md w-95-sm b-radius-10 paddinglr-40 paddingb-40">
						...
						<div class="center-align">
							<p class="text-19">{{ textModal }}</p>
						</div>
						<div class="center-align margin-auto margint-45">
							<div>
								<button @click="modal = false" class="paddinglr-45 marginlr-10 btn b-radius-25 red">Cancelar</button>
								<button @click="correctTest" class="paddinglr-45 marginlr-10 btn b-radius-25 red">Corregir</button>
							</div>
						</div>
...
export default {
	...
	this.limitTimeLocal = this.test.limitTime;
  	let time = setInterval(() => {
  		if (this.test.limitTime > 0) {
  			...
  			if (this.test.limitTime < 180) {
  				this.$refs.timer.classList.add('red')
  				this.$refs.timer.classList.remove('grey')
  				this.$refs.timer.classList.remove('darken-4')
  ...
  data() {
    return {
    	...
    	finishTest: false,
    	limitTimeLocal: 0,
    	textModal: "El test ha finalizado y se va a corregir",
    	testFinalized: true,
    };
  },
  methods: {
  	...
  	fillQuestion(index){
  		if (this.testFinalized) {
	  		this.question = this.questions[index];
	  		this.indexQuestions = index;
	  		this.updateChip(false);
  		}else{
  			M.toast({html: 'El test ha finalizado'});
  		}
  	},
